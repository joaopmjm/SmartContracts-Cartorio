...
def sign_contract_ok(DocumentSignSimple, accounts):
    # deploy the contract with the initial values as a constructor argument
    yield DocumentSignSimple.deploy(accounts[1], DOCUMENT, {'from': accounts[0]})


def test_initial_state(sign_contract_ok):
    # Check if the constructor of the contract is set up properly
    assert sign_contract_ok.GetDocument()[1] == False
    assert sign_contract_ok.GetSignedInfo()[1] == False 


def test_contract(sign_contract_ok, accounts):
    # Funding Test
    assert sign_contract_ok.GetSignedInfo()[1] == False  # Directly access donations
    assert sign_contract_ok.GetDocument()[1] == False
    sign_contract_ok.Sign({'from': accounts[1]})
    assert sign_contract_ok.GetSignedInfo()[1] == True  # Directly access donations
